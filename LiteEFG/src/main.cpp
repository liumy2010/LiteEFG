...
int main(){
    int N, M, type;
    std::cin>>N>>M>>type;
    static Environment* env = new FileEnvironment("GameInstances/kuhn.game", (type==0)?"Enumerate":"Outcome");
    ...
    auto gradient = GraphNode::Aggregate(ev, "sum") + graph.utility;
...
PYBIND11_MODULE(LiteEFG, m) {
    ...
    m.def("aggregate", py::overload_cast<const GraphNode&, const std::string&, const std::string&, const double&>(GraphNode::Aggregate), py::arg(), py::arg("aggregator"), py::arg("object")="children", py::arg("padding")=0.0);
    ...
    py::class_<Graph>(m, "Graph")
        .def(py::init<>())
        .def("backward", &Graph::BackwardNode, py::arg("is_static") = false)
        .def("forward", &Graph::ForwardNode, py::arg("is_static") = false)
    ...
    py::class_<Environment, std::shared_ptr<Environment>>(m, "Environment")
        .def("GetGraph", &Environment::GetGraph, py::arg("graph"))
        .def("Update", py::overload_cast<const GraphNode&, const int&>(&Environment::Update), py::arg("strategy"), py::arg("upd_player") = -1)
        .def("Update", py::overload_cast<std::vector<GraphNode>, const int&>(&Environment::Update), py::arg("strategies"), py::arg("upd_player") = -1)
        .def("UpdateStrategy", py::overload_cast<const GraphNode&, const bool&>(&Environment::UpdateStrategy), py::arg("strategy"), py::arg("update_best") = false)
